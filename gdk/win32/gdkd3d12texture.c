...
gdk_d3d12_texture_dispose (GObject *object)
{
  ...
  g_clear_pointer (&self->handle, CloseHandle);

  gdk_win32_clear (&self->resource);
...
#define FENCE_SIGNAL 1
  ...
  gdk_win32_clear (&buffer);
  gdk_win32_clear (&commands);
  gdk_win32_clear (&allocator);
  gdk_win32_clear (&queue);
  gdk_win32_clear (&fence);
  gdk_win32_clear (&device);
}
...
gdk_d3d12_get_format (DXGI_FORMAT dxgi_format, 
                      gboolean    premultiplied)
{
  ...
  for (format = 0; format < GDK_MEMORY_N_FORMATS; format++)
    {
      if (gdk_memory_format_dxgi_format (format, NULL) == dxgi_format &&
          ((premultiplied && gdk_memory_format_alpha (format) != GDK_MEMORY_ALPHA_STRAIGHT) ||
           (!premultiplied && gdk_memory_format_alpha (format) != GDK_MEMORY_ALPHA_PREMULTIPLIED)))
        break;
    }
...
HANDLE
gdk_d3d12_texture_get_handle (GdkD3d12Texture *self)
{
  ...
  result = g_atomic_pointer_get (&self->handle);
  if (result)
    return result;
  ...
  g_atomic_pointer_set (&self->handle, result);
                             ...
                             guint           *out_mem_id)
{
  ...
  handle = gdk_d3d12_texture_get_handle (self);
  if (!handle)
    return 0;
